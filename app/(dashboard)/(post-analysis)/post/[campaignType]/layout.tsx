...
export default function RootLayout({ children }: { children: ReactNode }) {
    ...
    const resetSearch = () => {
        setIsSearch(false);
        setSearchText('');
        dispatch(getCampaigns({ page: 1, limit: 12, status: CampaignStatus.active, ownerType: ownerType, q: '', type: "post" }));
    };

    const searhFilter = () => {
        if (searchText !== '') {
            setIsSearch(true);
            dispatch(getCampaigns({ page: 1, limit: 12, status: CampaignStatus.active, ownerType: ownerType, q: searchText, type: "post" }));
        } else if (searchText === '') {
            resetSearch();
        }
    };
                            ...
                            <div className='flex justify-end sm:justify-between items-center'>
                                ...
                                <div className='flex gap-3 justify-center items-center ml-16 sm:ml-0'>
                                    ...
                                    {paths.indexOf('campaign') > -1 && (
                                        <div className='flex'>
                                            <button
                                                onClick={() => copyShareLink(`/post/${params?.campaignType}/campaign/${params.campaignId}?isPublic=true`)}
