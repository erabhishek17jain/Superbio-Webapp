...
export default function AllCampaignPage() {
    ...
    const fetchMore = () => {
        ...
        dispatch(getCampaigns({ page: allCampaign?.meta?.arg?.page || 0 + 1, limit: 12, status: CampaignStatus.active, ownerType: ownedType, q: '', type: "influencer" }));
        dispatch(
            setMeta({
    ...
    const loadMore = async () => {
        if (allCampaign?.meta.page && allCampaign?.meta.page * 12 < allCampaign?.meta.total) {
            ...
            dispatch(getCampaigns({ page: allCampaign?.meta.page + 1, limit: 12, status: allCampaign?.meta.status, ownerType: ownerType, q: '', type: "influencer" }));
            setloader(false);
        }
    };
