...
export default function Home() {
  ...
  const { activeCampaign, sharedCampaign, loading } = useAppSelector(
  ...
  return (
    ...
    >
      <div className="flex w-full items-center justify-between pl-4 sm:pl-8 pr-4 py-3 border-[#cdcdcd] border-b h-16 z-10">
        ...
        <div className="flex text-base w-full justify-between items-center h-12 ml-2 sm:ml-0">
          ...
          <div className="flex gap-3 ml-16 sm:ml-0">
            ...
            {searchText !== "" && isSearch && (
              <div
                className="flex justify-between px-3 items-center bg-[#F7F7F7] rounded-lg"
                onClick={resetSearch}
              >
      ...
      <div className="flex px-4 sm:px-8 py-4 w-full h-full flex-col overflow-y-auto mb-10 sm:mb-0">
        {searchText !== "" && isSearch && (
          <div className="flex py-3 uppercase text-[#8b8b8b] text-sm">
            ...
            {activeCampaign && activeCampaign.data?.length > 0 && (
              <div className="flex flex-col w-full mb-4 sm:mb-10">
                <div className="flex w-full justify-between mb-3 sm:mb-5">
                  <span className="font-semibold text-xl text-opacity-80">
                    Active campaigns
                  </span>
                  <Link
                    href={`/active`}
                    className="text-[#8b8b8b] cursor-pointer text-base font-semibold"
                  >
                    See More
                  </Link>
                </div>
                {activeCampaign.data?.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 w-full gap-4">
                  ...
                  <div className="flex flex-col w-full">
                    <div className="flex h-48 justify-center items-center w-full text-[#8b8b8b]">
