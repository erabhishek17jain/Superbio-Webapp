...
export default function Home() {
    ...
    const { data: activeCampaign, isLoading } = useSWR(`?page=1&limit=3&status=active&ownerType=own${searchText !== '' ? '&q='+searchText : ''}`, CampaignNetworkService.instance.fetcher)
    const { data: sharedCampaignData } = useSWR(`?page=1&limit=3&status=active&ownerType=shared${searchText !== '' ? '&q='+searchText : ''}`, CampaignNetworkService.instance.fetcher)
    ...
    const shouldShowNoCampaign = (activeCampaign && activeCampaign.length > 0) || (sharedCampaignData && sharedCampaignData.length > 0) || (archivedCampaignData && archivedCampaignData.length > 0)
                        ...
                        {archivedCampaignData && archivedCampaignData?.length > 0 && (
                            <div className='flex flex-col w-full'>
                                <div className='flex w-full justify-between mb-5'>
                                    ...
                                    <Link href={'/archived-campaign'} className='text-[#676767] cursor-pointer text-base font-semibold'>
                                        See More
                                    </Link>
                                </div>
                                {archivedCampaignData?.length > 0 ? (
                                    <div className='grid grid-cols-1 md:grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 w-full gap-4'>
                                        {archivedCampaignData?.map((data, index) => (
                                            <CampaignCard key={index} campaign={data} setMode={editCampaign} status='shared-campaign' color={'#FEF7E6'} />
                                        ))}
                                    </div>
                                ) : (
                                    <div className='flex flex-col w-full'>
