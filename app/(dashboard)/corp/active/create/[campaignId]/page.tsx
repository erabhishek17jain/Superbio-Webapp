...
import { SheetDetails } from '@/components/shared-components/corp/SheetDetails';
...
export default function CreateReporting() {
    ...
    const mapColumns = () => {
        if (mode === 'view') {
            router.push(`/corp/active/report/${params.campaignId}`);
            return;
        }
        let error = false;
    ...
    const mapSingleSheet = async (item: any) => {
        ...
        const response = await CorpNetworkService.instance.mappingColumns(mapParams);
        ...
        for (let i = 0; i < sheetData.length; i++) {
            ...
            if (ind === -1) {
                ...
                if (i === sheetData.length - initialSheetData.length - 1) {
                    ...
                    await CorpNetworkService.instance.generateReport(param);
                    router.push(`/corp/${params?.campType}/report/${params.campaignId}`);
                }
            }
        }
    };
