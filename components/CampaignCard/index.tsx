...
import ArchiveCampaignModal from '../Modals/ArchiveCampaignModal';
...
export default function CampaignCard({ campaign, status, setMode, color }: { campaign: ICampaign; status: string; setMode: any; color: string }) {
    ...
    const navigateToReporting = () => {
        router.push(`/${status}/campaign-reporting/${campaign.id}?campaignName=${campaign.title.replaceAll(' ', '_')}`);
    };
    ...
    const campActions = [
        ...
        },
        {
            ...
            action: () => router.push(`/${status}/create-reporting/${campaign?.id}?campaignName=${campaign?.title.replaceAll(' ', '_')}`),
            icon: (
    ...
    return (
        <div className='flex w-full relative'>
            ...
            <div className={`flex flex-col w-full h-full bg-[${color}] rounded-xl py-4 px-4`}>
                ...
                <div onClick={navigateToReporting} className='cursor-pointer flex justify-between mt-1'>
                    <div>
                        <span className='text-[#959595] text-sm'>Brand: </span>
                        <span className='text-black text-sm font-semibold ml-1'>{campaign.brand}</span>
                    </div>
                    <div>
                        <span className='text-[#959595] text-sm'>Created At: </span>
                        <span className='text-black text-sm font-semibold ml-1'>{dayjs(campaign.startDate).format('DD MMM YYYY')}</span>
                    </div>
                </div>
                ...
                <div onClick={navigateToReporting} className='cursor-pointer mt-3 flex flex-wrap gap-2'>
                ...
                <div onClick={navigateToReporting} className='cursor-pointer flex justify-between mt-4'>
                    <span>
                        <span className='text-[#959595] text-sm'>Source: </span>
                    ...
                    <span>
                        <span className='text-[#959595] text-sm'>Created By: </span>
                        <span className='text-black text-sm font-semibold ml-1'>{campaign?.user?.email === user?.email ? 'You' : campaign?.user?.name}</span>
                    </span>
                </div>
                <div className='flex mt-3 fill-[#B3B3B3] items-center justify-between text-[#959595] text-sm h-7'>
            ...
            {showArchiveModal && <ArchiveCampaignModal campaign={campaign} openCloseModal={openCloseArchiveModal} />}
        </div>
    );
}
